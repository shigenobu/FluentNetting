...
public sealed class FnEventModeFormatter : IMessagePackFormatter<BaseFnEventMode>
{
    ...
    public void Serialize(ref MessagePackWriter writer, BaseFnEventMode value, MessagePackSerializerOptions options)
    {
        ...
        switch (value)
        {
            case FnMessageMode o:
                ...
                if (hasMessageModeOption) MessagePackSerializer.Serialize(ref writer, o.Option!);
            ...
            case FnForwardMode o:
                ...
                if (hasForwardModeOption) MessagePackSerializer.Serialize(ref writer, o.Option!);
            ...
            case FnPackedForwardMode o:
                ...
                if (hasPackedForwardModeOption) MessagePackSerializer.Serialize(ref writer, o.Option!);
    ...
    {
        if (reader.IsNil) throw new MessagePackSerializationException("Data is Nil, FnEventMode can not be null.");

        // To detect mode at first.
        var peekReader = reader.CreatePeekReader();

        var count = peekReader.ReadArrayHeader();
        if (count is < 2 or > 4) throw new MessagePackSerializationException("Invalid BaseFnEventMode count.");
            ...
            case MessagePackType.Binary:
                ...
                if (count == 3)
                {
                    ...
                    if (reader.NextMessagePackType == MessagePackType.Map)
                    {
                        ...
                        if (option.TryGetValue("compressed", out var type))
                        {

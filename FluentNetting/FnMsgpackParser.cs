...
internal static class FnMsgpackParser
{
    ...
    internal static bool TryParseForPing(byte[] message, out FnPing? ping)
    {
        ...
        try
        {
            ...
            // check PING format
            if (!reader.TryReadArrayHeader(out var count) || count != 6) return false;

            // 1st value
            if (reader.ReadString() != "PING") return false;
            ...
            // 5th value
            var username = reader.ReadStringSequence();

            // 6th value
            var password = reader.ReadStringSequence();

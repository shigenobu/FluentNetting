...
{
    private static readonly IReadOnlyDictionary<Type, Func<object, MessagePackObject>>
        _typedMessagePackObjectFactories = new Dictionary<Type, Func<object, MessagePackObject>>
        {
            ...
            {typeof(DateTime), obj => new MessagePackObject(MessagePackConvert.FromDateTime((DateTime) obj))},
            {
                ...
                obj => new MessagePackObject(MessagePackConvert.FromDateTimeOffset((DateTimeOffset) obj))
            }
        };
    ...
    private MessagePackObject CreateMessagePackObject(object value)
    {
        ...
        if (_typedMessagePackObjectFactories.TryGetValue(type.AsType(), out var factory)) return factory.Invoke(value);

        if (type.IsEnum) return new MessagePackObject(value.ToString());
        ...
        if (type.GetInterfaces().Any(x => x == typeof(IEnumerable)))
        {
            ...
            foreach (var item in (IEnumerable) value) list.Add(CreateMessagePackObject(item));
        ...
        if (type.IsAnonymous())
        {
            ...
            var dictionary = properties.ToDictionary(x => new MessagePackObject(x.Name),
                x => CreateMessagePackObject(x.GetValue(value)));
...
{
    private static readonly DateTime _epochDateTime = new(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
    private static readonly DateTimeOffset _epochDateTimeOffset = new(1970, 1, 1, 0, 0, 0, TimeSpan.Zero);
